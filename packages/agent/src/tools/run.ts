...
// Accept `device` and pass it via env so the Playwright config can use devices[HYPERTEST_DEVICE]
type ToolLike = (input: {
  headed?: boolean;
...
export function runTool(): ToolLike {
  return async (opts = {}): Promise<AgentResult> => {
    ...
    const relToArtifacts =
      path.relative(path.resolve(process.cwd(), "artifacts"), specAbs) || "generated.spec.ts";
    ...
    if (opts.device) env.HYPERTEST_DEVICE = opts.device; // NEW: used by playwright config
