...
function parseFlags(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith("--")) {
      ...
      else if (argv[i + 1] && !argv[i + 1].startsWith("-")) out[k.slice(2)] = argv[++i];
...
function saveArtifacts(results: Array<{ message: string; artifacts?: Record<string, string> }>) {
...
async function main() {
  ...
  switch (cmd) {
    case "agent": {
      ...
      if (flags.model) process.env.OPENAI_MODEL = String(flags.model);
      if (flags.key) process.env.OPENAI_API_KEY = String(flags.key);
