...
function stampedDir(root = "artifacts", prefix: "run" | "rec" | "verify" = "run") {
...
function saveArtifacts(
  results: Array<{ message: string; artifacts?: Record<string, string> }>,
  rootDir?: string
) {
...
function applyProviderEnv(flags: Record<string, string | boolean>) {
  if (flags.provider) process.env.HYPERTEST_PROVIDER = String(flags.provider);
  ...
  // Ollama
  if (flags["ollama-model"]) process.env.OLLAMA_MODEL = String(flags["ollama-model"]);
  if (flags["ollama-base"]) process.env.OLLAMA_BASE_URL = String(flags["ollama-base"]);

  // LM Studio
  if (flags["lmstudio-base"]) process.env.LMSTUDIO_BASE_URL = String(flags["lmstudio-base"]);
  if (flags["lmstudio-model"]) process.env.LMSTUDIO_MODEL = String(flags["lmstudio-model"]);
}
...
function echoProviderBanner() {
  ...
  if (p === "openai") {
    ...
    console.log(
      `[agent] provider=ollama model=${process.env.OLLAMA_MODEL || "llama3.2"} base=${process.env.OLLAMA_BASE_URL || "http://localhost:11434"}`
    );
  } else if (p === "lmstudio") {
    console.log(
      `[agent] provider=lmstudio model=${process.env.LMSTUDIO_MODEL || "(default)"} base=${process.env.LMSTUDIO_BASE_URL || "http://localhost:1234"}`
    );
  } else {
...
function applyAgentRunEnvFromFlags(flags: Record<string, string | boolean>) {
  if (typeof flags.headed !== "undefined") process.env.HYPERTEST_RUN_HEADED = String(Boolean(flags.headed));
  if (typeof flags.trace === "string") process.env.HYPERTEST_RUN_TRACE = String(flags.trace);
  if (typeof flags["base-url"] === "string") process.env.HYPERTEST_BASE_URL = String(flags["base-url"]);
  if (typeof flags.reporter === "string") process.env.HYPERTEST_RUN_REPORTER = String(flags.reporter);
...
function findLatestRunDir(artifactsRoot = "artifacts"): string | null {
...
function runPlaywrightOn(
  ...
  if (opts.baseURL) env.BASE_URL = opts.baseURL; // picked up in config
  if (opts.device) env.HYPERTEST_DEVICE = opts.device;
...
function promoteLatest(destPath = "tests/generated.spec.ts") {
  ...
  if (!latest) throw new Error('No artifacts found under ./artifacts. Generate a test first with: hypertest agent "<goal>"');
  const src = path.join(latest, "generated.spec.ts");
  if (!fs.existsSync(src)) throw new Error(`No generated.spec.ts in ${latest}.`);
...
function writeVerifySpec(): string {
  ...
  switch (cmd) {
    case "agent": {
      ...
      if (flags["and-run"]) {
        ...
        const device =
          (typeof flags.device === "string" && flags.device) ||
    ...
    case "record": {
      ...
      if (typeof flags.browser === "string") {
        ...
        if (b === "chromium" || b === "firefox" || b === "webkit") args.push(`--browser=${b}`);
      }
      if (typeof flags.device === "string") args.push(`--device=${String(flags.device)}`);
      if (typeof flags.viewport === "string") {
        const v = String(flags.viewport).replace("x", ",");
        args.push(`--viewport-size=${v}`);
      }
      if (typeof flags["base-url"] === "string") process.env.BASE_URL = String(flags["base-url"]);
